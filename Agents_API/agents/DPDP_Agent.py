...
from agents.Agents_Workers.DPDP_Act_Agent import *
...
def identity(state: OverallAgentsState_BNS_Act,config: RunnableConfig) -> OverallAgentsState_BNS_Act:
    ...
    state.update({
                ...
                "User_question_DPDP_ACT": Pre_suffix_obj.get_prefix() + last_message + Pre_suffix_obj.get_suffix_DPDP_Act(),
                })
    return state

class LegalGraphBuilder_DPDP:
    ...
    def add_edges(self):
        ...
        self.Graph.add_edge("DPDP_Act_generate_answer", END)
        self.Graph.add_edge("DPDP_dummy","DPDP_no_answer")
        self.Graph.add_edge("DPDP_no_answer",END)

    def build(self):
        self.add_nodes()
        self.add_edges()
        return self.Graph.compile(name="VLANC DPDP Parallel Law Agents")



config_DPDP = RunnableConfig(configurable={"tracing": True}, run_name="VLANC-DPDP-LEGAL-AGENT")
_graph_builder_DPDP = LegalGraphBuilder_DPDP(OverallAgentsState_DPDP_Act)
_app_DPDP = _graph_builder_DPDP.build()
_app_DPDP = _app_DPDP.with_config(config_DPDP)
