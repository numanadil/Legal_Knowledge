...
function mainInjectedScript() {
  const text = window.getSelection().toString().trim();
  const cachedResponses = {};

  // Remove existing viewer if any
  document.getElementById("highlight-viewer")?.remove();
  ...
  header.innerText = "Indian Legal Agents 🕵️‍♂️";
  ...
  textDiv.innerText = text || "Please Select text on which applicable section of Indian Act do you want to see?";
  ...
  const buttonToKey = {
    ...
    "BNS ACT": "BNS_Act_Agent_answer",
    "BSA ACT": "BSA_Act_Agent_answer",
    "Web Search": "Web_Search_answer",
    "DPDP ACT":"DPDP_Act_Agent_answer",
    "Reflection Agent":"Web_Bare_Act_Reflexion_answers"
  };
  ...
  const endpointMap = {
    ...
    "Web Search": "WS",
    "DPDP ACT":"DPDP",
    "Reflection Agent":"All"
  };

  ["IT ACT", "BNS ACT", "BSA ACT", "Web Search","DPDP ACT", "Reflection Agent"].forEach(name => {
    ...
    btn.onclick = async () => {
      ...
      if (cachedResponses[cacheKey]) {
        ...
        document.getElementById("content-box").innerHTML = marked(cached, { breaks: true });
        btnSpinner.style.display = "none";
        btnText.innerText = name;
        return;
      }
      ...
      try {
        ...
        const url = `ws://127.0.0.1:8001/ws/extension/${endpointMap[name]}/`;

        const ws = new WebSocket(url);


        ws.onopen = () => ws.send(JSON.stringify({ data: text }));
        ...
        ws.onmessage = (event) => {
          try {
            ...
            cachedResponses[cacheKey] = answer;
            document.getElementById("content-box").innerHTML = marked(answer, { breaks: true });
          } catch (e) {
            document.getElementById("content-box").innerHTML = "⚠️ Invalid response format.";
          }
          ws.close();
        };

        ws.onerror = () => {
          document.getElementById("content-box").innerHTML = "⚠️ WebSocket connection failed.";
        };
        ...
        document.getElementById("content-box").innerHTML = "⚠️ Failed to connect WebSocket.";
        btnSpinner.style.display = "none";
        btnText.innerText = name;
      }
    };
  ...
  function showPopup(name, answer) {
    document.getElementById("full-popup")?.remove();
